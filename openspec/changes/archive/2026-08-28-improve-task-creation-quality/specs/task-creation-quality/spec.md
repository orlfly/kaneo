...
### Requirement: requiredRole is mandatory for agent-created tasks

The system SHALL require `requiredRole` to be present (any of the seven agent roles or the literal `"human"`) whenever the create-task request is authenticated via an API key. Human session-cookie callers MAY continue to omit `requiredRole` to support legacy flows.

#### Scenario: Agent without requiredRole rejected

- **WHEN** an agent (authenticated with `x-api-key`) submits a create-task request without `requiredRole`
- **THEN** the API rejects the request with HTTP 400 and a message asking the agent to pick a role
