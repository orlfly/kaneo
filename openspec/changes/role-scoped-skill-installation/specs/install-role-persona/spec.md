...
### Requirement: All skills installed for every persona

**Reason**: The original install behavior bundled every SKILL.md with every persona regardless of role suitability, which conflicted with persona boundaries (e.g., `ui-design` persona receiving `submit-pr`). Role-scoped installation replaces this with explicit `for_roles` filtering.

**Migration**: Each SKILL.md must declare a `for_roles` frontmatter. Existing installations retain their previously copied SKILL.md files; re-running `install.sh` for a non-coding persona will remove skills that no longer match the persona when the script is invoked with the new role-scoping logic and `for_roles` frontmatter is present in the bundle.