...
description: Deliver your personal Kaneo notifications by email, ntfy, Gotify, or a custom webhook with team and project scoping
---
...
These settings are personal to the signed-in account. They do not change notification routing for other users in the team.
...
- delivery can be scoped by team and, optionally, by selected projects
...
Configure each channel once at the account level, then decide which teams are allowed to use it.
...
Below the global channel cards, Kaneo shows team delivery rules.
...
This lets a user receive notifications from one team in ntfy, another by email, and ignore the rest.
...
If Kaneo cannot resolve a notification to a team or project context, that notification stays inbox-only.
...
- a team rule exists for the notification team
- the team rule is active
- the channel is enabled on that team rule
...
- [Personalize Your Team](/core/functional/personalize-your-team)
