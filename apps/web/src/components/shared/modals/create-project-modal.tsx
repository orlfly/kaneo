...
function CreateProjectModal({ open, onClose }: CreateProjectModalProps) {
  ...
  const selectedTeam: ListTeamsResponse | undefined =
    teams.find((team) => team.id === selectedTeamId) ??
    teams.find((team) => team.id === activeTeam?.id);
  ...
  const handleClose = () => {
    ...
    setIconSearch("");
    setSelectedTeamId(activeTeam?.id ?? selectedTeam?.id ?? "");
    onClose();
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !selectedTeam?.id) return;
            ...
            <BreadcrumbList className="gap-1 text-xs">
              <BreadcrumbItem className="text-muted-foreground font-medium tracking-wide">
                {selectedTeam?.name?.toUpperCase() ||
                  t("common:modals.createProject.teamFallback")}
              </BreadcrumbItem>
        ...
        {teams.length === 0 ? (
          <div className="flex flex-col gap-3 px-3 pb-4">
            <p className="text-sm text-muted-foreground">
              {t("common:modals.createProject.noTeams")}
            </p>
            <Button
              type="button"
              onClick={() => {
            ...
            >
              {t("common:modals.createProject.manageTeams")}
            </Button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="px-3 pt-1">
              ...
              >
                <SelectTrigger
                  className="w-full"
                  aria-label={t("common:modals.createProject.teamLabel")}
                >
                  <SelectValue
                    placeholder={t(
                      "common:modals.createProject.teamPlaceholder",
                    )}
                  >
                    {selectedTeam?.name}
                  </SelectValue>
