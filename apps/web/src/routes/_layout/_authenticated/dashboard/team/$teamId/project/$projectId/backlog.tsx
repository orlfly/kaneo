...
function RouteComponent() {
  ...
  const { data: users } = useGetActiveTeamMembers(teamId);
  ...
  const toggleLabelGroup = (label: { name: string; color: string }) => {
    const matchingLabels = teamLabels.filter(
      (l: { name: string; color: string }) =>
        l.name === label.name && l.color === label.color,
    );
  ...
  return (
    <ProjectLayout projectId={projectId} teamId={teamId} activeView="backlog">
      <PageTitle
                  ...
                  <DropdownMenuContent className="w-80" align="start">
                    ...
                    {users?.map((member) => (
                      <DropdownMenuCheckboxItem
                        ...
                        onCheckedChange={(checked) =>
                          updateFilter("assignee", checked ? member.id : null)
                        }
