...
function RouteComponent() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    reorderProjects(teamId, reordered, {
      onError: () => {
        setDroppedOrder(null);
  ...
  if (isLoading) {
    return (
      <>
        <PageTitle title={t("team:projects.pageTitle")} />
        <TeamLayout
          title={t("team:projects.pageTitle")}
          headerActions={
            canCreate ? (
              <Button
                variant="outline"
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-foreground font-medium">
                  {t("team:projects.progress")}
                </TableHead>
                <TableHead className="text-foreground font-medium">
                  {t("team:projects.targetDate")}
                </TableHead>
                <TableHead className="text-foreground font-medium">
                  {t("team:projects.status")}
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
        ...
        >
          <Empty className="min-h-[60vh]">
            <EmptyHeader>
              ...
              <EmptyTitle>{t("team:projects.emptyTitle")}</EmptyTitle>
              <EmptyDescription>
                {canCreate
                  ? t("team:projects.emptyDescription")
                  : t("team:projects.emptyDescriptionReadOnly")}
              </EmptyDescription>
            </EmptyHeader>
            <EmptyContent>
              {canCreate && (
                ...
                {orderedProjects?.map((project) => {
                  ...
                  const getStatusText = () => {
                    if (project.statistics.totalTasks === 0)
                      return t("team:projects.projectStatus.notStarted");
                    if (project.statistics.completionPercentage === 100)
                      return t("team:projects.projectStatus.complete");
                    return t("team:projects.projectStatus.inProgress");
                  };
