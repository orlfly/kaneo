...
- Keep schedules consistent with dependencies: if this task is blocked by another (or is a subtask that must finish before its parent), set its startDate on or after the blocker's dueDate. Call list_tasks first to see when related tasks are scheduled instead of stacking everything on the same days.
- Write a real description with acceptance criteria: what the implementer must do, context they need, and testable "验收标准/Acceptance Criteria" bullets. A title alone is not a task; the executing agent cannot read your mind or this chat.
- Put priority in the priority field (low/medium/high), not in the title. Do not prefix titles with metadata like "[coding-P2]"; put the role in requiredRole and the urgency in priority. Keep titles a short human-readable summary of the work itself.
- Break large work into smaller tasks: a task estimated to take more than ~3 days, or with more than ~5 unrelated acceptance criteria, should be split into a parent with subtasks (relationType 'subtask'), each with its own schedule and role.
...
/**
 * Interval between SSE comment heartbeats. Long tool executions (cloning a
 * repo, running commands) leave the stream idle for minutes; reverse proxies
 * and load balancers (nginx default proxy_read_timeout is 60s) drop idle
 * connections, which used to end the stream silently on the client while the
 * server kept working. Comments keep the connection alive without emitting
 * chat events the client renders.
 */
export const SSE_HEARTBEAT_INTERVAL_MS = 15_000;

/**
 * Start a heartbeat that writes an SSE comment every interval. Returns a stop
 * function; must be called when the response finishes so the timer does not
 * keep the process alive.
 */
export function startSseHeartbeat(stream: SSEStreamingApi): () => void {
  const timer = setInterval(() => {
    // SSE comment line: ignored by EventSource-compatible parsers and by our
    // own client parser (lines not starting with "data: " or "event: ").
...
async function sendMessageInner(
  ...
  const { projectId, userId, assistantId, messages } = params;
  ...
  while (round < MAX_TOOL_ROUNDS) {
    if (stream.aborted) {
      console.log("[chat] client disconnected; stopping pi-agent rounds");
      return;
    }
