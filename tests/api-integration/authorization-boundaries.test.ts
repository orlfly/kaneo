...
describe("github integration routes are team scoped", () => {
  it("refuses to list repositories for a non-member", async () => {
    const { user, team } = await createTeamMember({ role: "member" });
  ...
  it("refuses to verify an installation for a non-member", async () => {
    const { user, team } = await createTeamMember({ role: "member" });
    const { project } = await createProjectFixture({
      teamId: team.id,
    });
  ...
  it("refuses to list repositories for a project in another team", async () => {
    const outsider = await createTeamMember({ role: "owner" });
    const other = await createTeamMember({ role: "owner" });
    const { project } = await createProjectFixture({
...
describe("task assignees stay inside the team", () => {
  it("refuses to create a task assigned to a non-member", async () => {
    const { user, team } = await createTeamMember({ role: "owner" });
    const outsider = await createTeamMember({ role: "owner" });
    const { project } = await createProjectFixture({
      teamId: team.id,
    });
...
describe("activity attribution", () => {
  it("credits the session user, not a userId supplied in the body", async () => {
    const { user, team } = await createTeamMember({ role: "owner" });
    const impersonated = await createTeamMember({ role: "owner" });
    const { project, columns } = await createProjectFixture({
      teamId: team.id,
    });
...
describe("every assignee write path is team scoped", () => {
